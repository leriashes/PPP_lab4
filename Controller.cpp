...
bool Controller::getCameraData()
{
	...
	if (data.getNumber() % 3)
	{
		cout << "\nКОНТРОЛЛЕР .2_2: получено изображение с Камеры - обнаружено препятствие";
		return true;
	}
	else
	{
		cout << "\nКОНТРОЛЛЕР .2_2: получено изображение с Камеры - препятствий нет";
		return false;
	}
}
